...
//       if (peliculas[i].popularity > popularmax) {
//          popularmax = peliculas[i].popularity;
//          index = i;
//       }
//    }
//    return peliculas[index].title;
// }
// console.log(peliculaPopular());
// funcion que retorna le nombre de la pelicula con mas genero
function conMasGeneros() {
   let almacenmaximos = 0;
   let elcontador = 0;
   let index = [];
   let listMismasCantidades = [];
   for (let i = 0; i < peliculas.length; i++) {
      if (peliculas[i].genre_ids.length > almacenmaximos) {
         almacenmaximos = peliculas[i].genre_ids.length;
         listMismasCantidades.push(peliculas[i].genre_ids.length);
         index.push(i);
      }
   }
   if (
      listMismasCantidades[i] ===
      listMismasCantidades[listMismasCantidades.length - 1]
   ) {
      elcontador = listMismasCantidades[i];
      console.log(peliculas[index].title);
   }
}
console.log(conMasGeneros());
